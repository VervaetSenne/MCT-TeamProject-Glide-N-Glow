...
#region Subscriptions
    ...
    public async Task SetRgb(string macAddress,int r, int g, int b, CancellationToken cancellationToken, int anim = 1)
    {
    ...
    public async Task SetRgb(string macAddress, Color color,CancellationToken cancellationToken , int anim = 1)
    {
        await SetRgb(macAddress, color.R, color.G, color.B, cancellationToken, anim);
    }
    
    public async Task SetRgb(int buttonId, int r, int g, int b, CancellationToken cancellationToken, int anim = 1)
    {
        var macAddress = GetMacAddress(buttonId);
        await SetRgb(macAddress, r, g, b, cancellationToken, anim);
    }
    
    public async Task SetRgb(int buttonId, Color color, CancellationToken cancellationToken, int anim = 1)
    {
        await SetRgb(buttonId, color.R, color.G, color.B, cancellationToken, anim);
    }
    
    public async Task SetAllRgb(int r, int g, int b, CancellationToken cancellationToken, int anim = 1)
    {
        foreach (var esp in LightButtons)
        {
    ...
    public async Task SetAllRgb(Color color, CancellationToken cancellationToken , int anim = 1)
    {
        foreach (var esp in LightButtons)
        {
